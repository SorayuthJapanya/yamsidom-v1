...
import { useEditSpecies, useGetOneSpecies } from "../../api/admin/SpeciesApi";
...
const EditSpeciesForm = () => {
  ...
  const { data: speciesData } = useGetOneSpecies({ speciesId: id });

  const { mutate: editSpecies, isLoading } = useEditSpecies({
    setFormData,
    setImage,
    navigate,
    speciesId: id,
  ...
  useEffect(() => {
    if (speciesData) {
      setFormData({
        ...
        familyName: speciesData.familyName || "",
        description: speciesData.description || "",
        propagation: speciesData.propagation || "",
        plantingseason: speciesData.plantingseason || "",
        harvestingseason: speciesData.harvestingseason || "",
        utilization: speciesData.utilization || "",
        status: speciesData.status || "",
        surveysite: speciesData.surveysite || "",
      });
      if (speciesData.imageUrl) {
        setImage(speciesData.imageUrl);
      }
    }
  }, [speciesData]);
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 w-full">
          ...
          >
            <img
              src={
                image
                  ? `${import.meta.env.VITE_SERVER_URL}/uploads/species/${image}`
                  : "/leaf.png"
              }
              alt="Preview"
